...
Route::get('/debug-doc', function () {
    $disk = config('documents.disk');
    $root = config('documents.path');

    // Résolution du chemin réel
    $absoluteRoot = Storage::disk($disk)->path('');
    $realRoot     = realpath($absoluteRoot);

    // Test d'écriture
    $testFile = 'debug_test_' . time() . '.txt';
    $testPath = '2026/09/' . $testFile;

    $writeOk = false;
    $writeError = null;

    try {
        $writeOk = Storage::disk($disk)->put($testPath, 'Hello from debug');
    } catch (\Throwable $e) {
    ...
    return response()->json([
        'disk'                => $disk,
        'DOC_PATH_from_config'=> $root,
        'Storage_root'        => $absoluteRoot,
        'Storage_root_real'   => $realRoot,
        'root_is_dir'         => is_dir($absoluteRoot),
        'root_is_writable'    => is_writable($absoluteRoot),
        'dossier_2026_09'     => is_dir($absoluteRoot . '/2026/09'),
        'write_test'          => $writeOk,
        'write_test_path'     => $testPath,
        'file_exists_after'   => file_exists($absoluteRoot . '/' . $testPath),
        'write_error'         => $writeError,
        'open_basedir'        => ini_get('open_basedir'),
        'php_user'            => function_exists('posix_getpwuid') ? posix_getpwuid(posix_geteuid())['name'] : 'N/A',
        'base_path'           => base_path(),
