...
class AssignAdminPrestationPermissionsSeeder extends Seeder
{
    ...
    {
        $role = Role::where('code', 'admin_prestation')->first();

        if (!$role) {
            $this->command->warn('⚠️  Le rôle "admin_prestation" n\'existe pas. Exécutez d\'abord le seeder des rôles.');
            return;
        }
    ...
    private function assignPermissionsToRole(Role $role, $permissions): void
    {
        ...
        $this->command->info("✅ {$assigned} permissions assignées pour le rôle admin_prestation");
        if ($skipped > 0) {
