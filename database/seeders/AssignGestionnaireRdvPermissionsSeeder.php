...
class AssignGestionnaireRdvPermissionsSeeder extends Seeder
{
    ...
    {
        $role = Role::where('code', 'gestionnaire_rdv')->first();

        if (!$role) {
            $this->command->warn('⚠️  Le rôle "gestionnaire_rdv" n\'existe pas. Exécutez d\'abord le seeder des rôles.');
            return;
        }
    ...
    private function assignPermissionsToRole(Role $role, $permissions): void
    {
        ...
        $this->command->info("✅ {$assigned} permissions assignées pour le rôle gestionnaire_rdv");
        if ($skipped > 0) {
