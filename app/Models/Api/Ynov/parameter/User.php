...
class User extends Authenticatable
{
    ...
    /**
     * Obtenir la méthode 2FA configurée
     */
    public function getTwoFactorMethod(): string
    {
        return $this->two_factor_method ?? 'none';
    }

    /**
     * Vérifier si l'utilisateur est verrouillé pour la 2FA
     */
    ...
    /**
     * Obtenir le temps restant de verrouillage 2FA
     */
    public function getTwoFactorLockRemaining(): int
    {
        if (!$this->isTwoFactorLocked()) {
            return 0;
        }
        return (int) now()->diffInSeconds($this->two_factor_locked_until);
    }

    /**
     * Incrémenter les tentatives 2FA
     */
    ...
    /**
     * Réinitialiser les tentatives 2FA
     */
    public function resetTwoFactorAttempts(): self
    {
     ...
     * Activer la 2FA
     */
    public function enableTwoFactor(string $method, string $secret, array $recoveryCodes): self
    {
        $this->two_factor_enabled = true;
        $this->two_factor_method = $method;
        $this->two_factor_secret = $secret;
        $this->two_factor_recovery_codes = encrypt(implode(',', $recoveryCodes));
        $this->two_factor_attempts = 0;
        $this->two_factor_locked_until = null;
        $this->save();
