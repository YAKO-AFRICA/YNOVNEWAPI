...
class SignatureController extends Controller
{
    ...
    /**
     * Générer un lien de signature pour un gestionnaire (envoi au client)
     * 
     * Ce endpoint est spécifique au scénario où un gestionnaire en agence
     * génère un lien de signature pour l'envoyer au client par email/SMS/WhatsApp
     * 
     * @param Request $request
     * @return JsonResponse
     * 
     * @bodyParam document_url string nullable URL du document
     * @bodyParam client_email string required Email du client
     * @bodyParam client_phone string nullable Téléphone du client
     * @bodyParam client_name string nullable Nom du client
     * @bodyParam document_description string nullable Description du document
     * @bodyParam send_method string required Méthode d'envoi : email, sms, whatsapp
     * @bodyParam webhook_url string required URL de réception de signature
     * @bodyParam api_key string required Secret partagé
     * 
     * @response 201 {"success":true,"message":"Lien généré et envoyé","code":"MANAGER_LINK_SENT","data":{"token":"...","widget_url":"...","sent_via":"email"}}
     */
    public function generateManagerLink(Request $request): JsonResponse
    {
        try {
            $validated = $request->validate([
                'document_url' => ['nullable', 'url'],
                ...
                'document_description' => ['nullable', 'string', 'max:500'],
                'send_method' => ['required', 'in:email,sms,whatsapp'],
                'webhook_url' => ['required', 'url'],
                'api_key' => ['required', 'string'],
            ...
            $result = $this->signatureService->generateSignatureLink(
                ...
                $validated['webhook_url'],
                $validated['api_key'],
                null, // Pas de redirect pour ce scénario
                null, // Pas de cancel redirect
                false, // Pas de polling auto
                $validated['expires_in'] ?? 7200 // 2h par défaut pour ce scénario
            );

            // Envoyer le lien selon la méthode choisie
            $sendResult = null;
            $sendMethod = $validated['send_method'];
            
            switch ($sendMethod) {
                case 'email':
                    $sendResult = $this->signatureService->sendLinkByEmail(
                        $result['token'],
                        $validated['client_email'],
                        'Document à signer - ' . ($validated['client_name'] ?? 'Cher client'),
                        "Bonjour" . ($validated['client_name'] ? " {$validated['client_name']}" : '') . ",\n\nVeuillez trouver ci-joint le document à signer en cliquant sur le lien suivant :\n\n{$result['widget_url']}\n\nCe lien est valide pour 2 heures.\n\nCordialement,\nL'équipe YAKOA AFRICASSUR"
                    );
                    break;
                ...
                case 'sms':
                    if (!$validated['client_phone']) {
                        return response()->json([
                            'success' => false,
                            'message' => 'Numéro de téléphone requis pour SMS.',
                            'code' => 'PHONE_REQUIRED',
                        ], 422);
                    }
                    $sendResult = $this->signatureService->sendLinkBySms(
                        $result['token'],
                        $validated['client_phone'],
                        "Signez votre document ici : {$result['widget_url']}"
                    );
                    break;
                ...
                case 'whatsapp':
                    if (!$validated['client_phone']) {
                        return response()->json([
                            'success' => false,
                            'message' => 'Numéro de téléphone requis pour WhatsApp.',
                            'code' => 'PHONE_REQUIRED',
                        ], 422);
                    }
                    $sendResult = $this->signatureService->sendLinkByWhatsapp(
                        $result['token'],
                        $validated['client_phone'],
            ...
            if (!$sendResult['success']) {
                return response()->json([
                    'success' => false,
                    'message' => 'Lien généré mais erreur d\'envoi : ' . $sendResult['message'],
                    'code' => $sendResult['code'],
                    'data' => $result, // Retourner quand même le lien généré
                ], $sendResult['status'] ?? 500);
            }

            return response()->json([
                'success' => true,
                'message' => 'Lien de signature généré et envoyé par ' . $sendMethod . '.',
                'code' => 'MANAGER_LINK_SENT',
                'data' => array_merge($result, [
                    'sent_via' => $sendMethod,
                    'sent_to' => $sendMethod === 'email' ? $validated['client_email'] : $validated['client_phone'],
            ...
            return response()->json([
                'success' => false,
                'message' => 'Erreur lors de la génération du lien gestionnaire.',
                'code' => 'MANAGER_LINK_ERROR',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
