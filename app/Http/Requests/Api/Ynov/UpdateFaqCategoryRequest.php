<?php
// app/Http/Requests/Api/Ynov/UpdateFaqCategoryRequest.php
...
class UpdateFaqCategoryRequest extends FormRequest
{
    ...
    {
        return $this->user()?->hasPermission('faq_categories.modifier') ?? false;
    }

    public function rules(): array
    {
        $category = $this->route('faq_category');
        $uuid = $category ? $category->uuid_faq_category : null;

        return [
            'label' => ['sometimes', 'string', 'max:100'],
            'code' => ['nullable', 'string', 'max:50', "unique:faq_categories,code,{$uuid},uuid_faq_category"],
